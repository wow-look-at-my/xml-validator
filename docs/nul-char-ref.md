...
The XML 1.1 `Char` production starts at `#x1`, so a strict reading rejects a reference that resolves to U+0000. This validator accepts it. `IsCharRefValue` in `validator/chars.go` is `IsChar` plus U+0000. It is used only where a character reference resolves: content and attribute values. A literal NUL byte is still rejected everywhere: character data, attribute values, CDATA sections and comments. So the reference is the only way U+0000 enters a parsed value. That is what keeps the byte stream free of NUL bytes by construction.

A lone surrogate stays rejected too. It is not a character in any encoding, so there is nothing for a reference to resolve to.
...
`serializeDoc` is a test helper, not library API. It emits no namespace declarations, so every document in that file declares none.

### Executable roundtrip -- `dats/nul-char-ref.dats`

The tests here drive the built binary, so nothing depends on a Go caller. `go-toolchain` runs them after every build, and there is no opt-out. Two answer the terminator claim directly:
...
Measuring the 256-character payload turned up a real defect. The `length`, `minLength` and `maxLength` facets counted the bytes of the UTF-8 encoding, so the payload reported as 384. XSD defines the unit as characters for a string type, and octets for `xs:hexBinary` and `xs:base64Binary`. `validator/schema_facets.go` now counts each of those. A facet value that is not a non-negative integer is an error in the schema rather than a silent length of 0. `validator/schema_facet_length_test.go` covers each case.
...
The suite is sandboxed (bubblewrap, falling back to docker). A host with neither fails the run rather than running the commands unsandboxed.
